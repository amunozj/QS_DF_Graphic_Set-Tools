    ...
    if ncr>0
        ...
        for ipgs = 1:npgs
            ...
            if names_sw
                ...
                ImgO = uint8(zeros(length(catg_humanoids)*tlsz(1)+2+max(szn1),pgsz*tlsz(2)+2+max(szc2),3));
                %         ImgO(:,:,:) = 255;
                TrnsO = uint8(ones(length(catg_humanoids)*tlsz(1)+2+max(szn1),pgsz*tlsz(2)+2+max(szc2),1)*0);
                ...
                %Horizontal
                ImgO(:,pgsz*tlsz(1)+1,1) = 255;
                ImgO(:,pgsz*tlsz(1)+2,2) = 255;
                ImgO(:,pgsz*tlsz(1)+3:size(ImgO,2),:,:) = 255;
                TrnsO(:,pgsz*tlsz(1)+1:size(ImgO,2),:) = 255;
                ...
                for i = 1:pgsz
                    ...
                    ImgO(length(catg_humanoids)*tlsz(1)+3:length(catg_humanoids)*tlsz(1)+2+szn1(i),(i-1)*tlsz(2)+1+round(dtx/2):i*tlsz(2)-(dtx-round(dtx/2)),:) = names(pgin(1)-1+i).text;
                end
                ...
                for i = 1:length(catg_humanoids)
                    ...
                    ImgO((i-1)*tlsz(1)+1+round(dtx/2):i*tlsz(1)-(dtx-round(dtx/2)), pgsz*tlsz(2)+3:pgsz*tlsz(2)+2+szc2(i),:) = catg_humanoids(i).text;
                end
            ...
            if npgs==1
                ...
                FileO = [FolderO 'raw\graphics\QS_STD\' fl_name num2str(ipgs) '_' num2str(tlsz(1)) 'x' num2str(tlsz(2)) '.png'];
            end
