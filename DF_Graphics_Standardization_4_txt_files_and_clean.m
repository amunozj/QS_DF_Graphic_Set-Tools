...
for ifR = 1:length(FlsR)
    ...
    if ncr>0
        ...
        fl_name3 = ['QS_STD\' FlsR(ifR).name(max(slsh_in)+1:length(FlsR(ifR).name)-4)];
        fl_name3 = strrep(fl_name3, 'creature', 'QS_STD_CRT');
...
for ifR = 1:length(FlsR)
    ...
    if ncr>0
        ...
        fl_name3 = ['QS_STD\' FlsR(ifR).name(max(slsh_in)+1:length(FlsR(ifR).name)-4)];
        fl_name3 = strrep(fl_name3, 'creature', 'QS_STD_PRSN');
...
for ifR = 1:length(FlsR)
    ...
    if ncr>0
        ...
        for icr = 1:ncr
            ...
            fl_name3 = ['QS_STD\' FlsR(ifR).name(max(slsh_in)+1:length(FlsR(ifR).name)-4)];
            fl_name3 = strrep(fl_name3, 'creature', ['QS_STD_MJR_' lower(nameR)]);
...
FlsStd = rdir([FolderO 'raw\graphics\**\graphics_QS_STD*.txt']);
